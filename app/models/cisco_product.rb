...
class CiscoProduct
  ...
  def hw_price
    ((full_price - base_price) / 0.8) / 12.0
  end
  ...
  def description
    ...
    @description = Nokogiri::HTML(Net::HTTP.post_form(URI.parse(description_url), {'product' => URI.escape(part_number), 'Submit' => 'Find+Cisco+Product+to+provide+cover+for..'}).body).xpath(@description_xpath)
